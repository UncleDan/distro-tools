...
# Estrae le righe di repository relative a Google Chrome e la/le
# chiave/i GPG corrispondenti nella cartella "chromerepo" creata
# accanto a questo script, con proprietario e permessi dell'utente reale.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUT_DIR="$SCRIPT_DIR/chromerepo"
mkdir -p "$OUT_DIR"
...
REPO_FILE="$OUT_DIR/chrome.sources.list"
> "$REPO_FILE"

echo "Cerco il repository Google Chrome nei file apt..."

declare -a KEY_PATHS=()

for f in /etc/apt/sources.list /etc/apt/sources.list.d/*.list /etc/apt/sources.list.d/*.sources; do
    [ -f "$f" ] || continue
    if sudo grep -qi "chrome" "$f" 2>/dev/null; then
        {
            echo "# --- da: $f ---"
            sudo grep -i "chrome" "$f"
            echo ""
        } >> "$REPO_FILE"
