...
# cartella "vscoderepo" prodotta da esporta-vscode.sh.
...
SRC_DIR="$SCRIPT_DIR/vscoderepo"

if [ ! -d "$SRC_DIR" ]; then
    echo "Cartella 'vscoderepo' non trovata accanto a questo script."
    exit 1
fi

REPO_FILE="$SRC_DIR/vscode.sources.list"

if [ ! -s "$REPO_FILE" ]; then
    echo "Nessun repository Visual Studio Code da registrare (file vuoto o assente)."
    exit 0
fi

DEST_LIST="/etc/apt/sources.list.d/vscode.list"
...
echo ""
echo "Fatto. Verifica con: apt-cache policy | grep -i code"
